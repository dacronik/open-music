...
rap:[
  ...
  getters: {
    totalPop:state =>state.pop.length,
    totalRock:state=>state.rock.length,
    totalRap:state=>state.rap.length,
    totalAlbums:state=>{
      return state.rap.length + state.pop.length + state.rock.length
    }

  },  
  mutations: {
    ADD_ALBUM:(state, newAlbum)=>{
      newAlbum.id = uuid()//Math.floor(Math.random()*1000)
      if(newAlbum.category == 'Pop'){
        state.pop.push(newAlbum)
      }else if(newAlbum.category == 'Rock'){
        state.rock.push(newAlbum)
      }else if(newAlbum.category == 'Rap'){
        state.rap.push(newAlbum)
      }
    }
  },
  actions: {
    addAlbum:({commit},newAlbum) =>{
      commit('ADD_ALBUM', newAlbum)
    }
  },
  modules: {
  }
